...
**Arreglado**: los 82 usos de `UUID.fromString(jwt.getClaimAsString("empresa_id"))` en 17 controllers
...
`NotificacionController` y `PlantillaNotificacionController` se hicieron en una segunda pasada, tras
mergear el PR #150 (se dejaron fuera de la primera para no chocar con él). Ahí `listar` se migró al
opcional `tenant.empresaId()`, **no** a `empresaIdRequerida()`: es una lista y debe seguir devolviendo
vacío sin empresa, no 403.

**Qué NO es el bug**: quedan 20 usos de `jwt.getClaimAsString("empresa_id")` que asignan a `String` y
**comprueban el null explícitamente** (listas que devuelven vacío, `SucursalController` validando la
ruta contra el token, `CarritoController` resolviendo si el actor es cliente o personal). Verificado uno
por uno: ninguno parsea el claim sin protección. Esos se dejan como están.
...
## BACK-3 — perfil propio y cambio de contrasena (2026-07-22)

El usuario no tenia forma de editar sus datos ni de cambiar su contrasena estando dentro: la unica via
era el correo de recuperacion. La pantalla "Perfil" del cliente solo podia mostrar el correo.

- **V66**: `usuario` gana `nombre` y `telefono` (opcionales). Van en el **usuario**, no en la ficha de
  cliente: son suyos y valen para todas las tiendas, mientras que la ficha es por empresa y la administra
  el dueno.
- Dominio: `Usuario.actualizarPerfil(...)` y `nombreParaMostrar()` (cae al correo si aun no cargo nombre).
- Puerto `GestionarPerfilPropio` + `PerfilPropioService`.
- Endpoints: **`GET /api/v1/perfil`**, **`PUT /api/v1/perfil`**, **`POST /api/v1/perfil/contrasena`**.
  Siempre operan sobre el usuario del token — no reciben id, asi que nadie edita la cuenta de otro. Con
  `anyRequest().authenticated()` ya quedan disponibles para cliente, empleado y dueno sin tocar reglas.
- Cambiar la contrasena **exige la actual** aunque la sesion este abierta: si no, quien agarre un telefono
  desbloqueado podria dejar al dueno fuera de su cuenta. Mismo minimo de 8 caracteres que al restablecer
  por correo (la regla no puede ser mas floja por otra puerta).
- Tests: `PerfilPropioIntegrationTest` (4 casos). **Suite 513/513.**

**Pendiente relacionado (no incluido):** la ficha que se le crea al cliente al comprar en una tienda sigue
...
| 5 | ✅ **HECHO (BACK-3).** ~~Perfil del cliente vacio.~~ No puede editar sus datos ni cambiar contrasena: no hay endpoint para que el propio usuario lo haga. | **BACK-3** | FRONT-5 |
