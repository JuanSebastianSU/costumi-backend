 ...
 */
public record RentaResponse(UUID id, String codigoRetiro, UUID sucursalId, UUID clienteId, UUID empleadoId,
		List<LineaRentaResponse> lineas, UUID prendaId, LocalDate fechaRetiro, LocalDate fechaDevolucion,
		BigDecimal precioPorDia, BigDecimal deposito, BigDecimal importe, String estado) {
	...
	static RentaResponse desde(Renta r, Map<UUID, ResumenDePrenda> resumenes) {
		List<LineaRentaResponse> lineas = r.lineas().stream()
				...
				.toList();
		return new RentaResponse(r.id(), CodigoDeRetiro.de("R", r.id()), r.sucursalId(), r.clienteId(), r.empleadoId(),
				lineas, r.prendaId(), r.fechaRetiro(), r.fechaDevolucion(), r.precioPorDia(), r.deposito(), r.importe(),
				r.estado().name());
	}
}
