...
/**
 * Datos para rentar VARIOS disfraces (distintos, con cantidad) al mismo cliente en una sola renta. Cada
 * {@code item} resuelve un disfraz a sus piezas; todas las líneas se acumulan en una única renta.
 */
public record RentarVariosDisfracesComando(UUID empresaId, UUID sucursalId, UUID clienteId, LocalDate fechaRetiro,
		LocalDate fechaDevolucion, List<ItemDeDisfraz> items, UUID empleadoId) {
