...
interface GrupoDeStockJpaRepository extends JpaRepository<GrupoDeStockJpaEntity, UUID> {
	...
	List<GrupoDeStockJpaEntity> findByPrendaIdAndSucursalId(UUID prendaId, UUID sucursalId);

	List<GrupoDeStockJpaEntity> findByEmpresaIdAndSucursalId(UUID empresaId, UUID sucursalId);
